...
public class BeautyPanelController {
  ...
  private String mCurrentFunction =
      null; // 当前选中的功能（如：white, dark, smooth, ai, lipstick, blush 等）
      ...
      if (imageView != null) {
        ...
        if (isOn) {
          ...
          android.graphics.PorterDuffColorFilter filter =
              new android.graphics.PorterDuffColorFilter(0xFFFFEEEE, // 稍微偏暖的白色
      ...
      if (!config.enabled) {
        ...
        View button = createFunctionButtonFromConfig(config, v -> {
          android.widget.Toast
              .makeText(mRootView.getContext(), config.label + "功能开发中，敬请期待 😊",
